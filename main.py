...
class MainWindow(QWidget):
    ...
    def fill_album_table(self):
        ...
        for album in vkaudio.get_albums_iter():
            ...
            self.album_table.setItem(albumRow, 0, QTableWidgetItem(album['author']))
            self.album_table.setItem(albumRow, 1, QTableWidgetItem(album['title']))
            self.album_table.setItem(albumRow, 2, QTableWidgetItem(str(album['id'])))
            self.album_table.setItem(albumRow, 3, QTableWidgetItem(str(album['owner_id'])))
            self.album_table.setItem(albumRow, 4, QTableWidgetItem(album['access_hash']))
    
    def download_button_click(self):
        if self.vk_album is not None:
            self.album_form = AlbumForm(self.vk_album)
